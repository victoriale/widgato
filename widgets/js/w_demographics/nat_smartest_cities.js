...
$(function(){
  ...
  function dataCall(index){
  	$.get('//apirt.synapsys.us/index.php?widget=national-demographics&wid=11&skip='+index+'&limit=1', function(data){
      var curData = data.widget;
      var popData = curData[0].population;
      dataLength = curData.length;
      var title = "nat-smartest-cities";
      $('.fcw-t1').html('Cities in the U.S. with the Most Bachelor Degree Holders');
      $('.fcw-t2-loc').html(curData[0].DemoCity + ', ' + curData[0].DemoState);
      $('.fcw-img2').html('#'+(index+1));
      $('.fcw-content1').html(Number(curData[0].DemoSmartest).toFixed(0) + ' % of Residents');
