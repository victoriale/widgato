(function () {
    ...
    function setupIframe() {
        ...
        var outsideStyle = document.createElement("style");
        // style.type = 'text/css';
        ...
        if (query.wide != null && query.wide != '') {
            ...
            outsideStyle.innerHTML = ".widget_zone.is_big{right:0!important;} .widget_zone.is_small{right:calc(50% - 150px)!important;} #trivia_ad_zone{pointer-events: none;width: 300px;height: 250px;position: absolute;top: 0;right: calc(50% - 150px);} #ad_progress_fill {width: 100%;position: absolute;bottom: 0;z-index: 100;height: 3px;background: #000;opacity: .1;} #ad_progress_bar {position: absolute;bottom: 0;z-index: 100;height: 3px;background: #3098ff;background: -webkit-linear-gradient(left, #3098ff 0%, #3098ff 0%, #34d4ff 100%); background: linear-gradient(to right, #3098ff 0%, #3098ff 0%, #34d4ff 100%);}";
            ...
            var widgetZone = friendlyIframe.parentElement.getElementsByClassName("widget_zone")[0];
            if(widgetZone){
                widgetZone.parentElement.appendChild(triviaAdZone);
    ...
    var triviaWidget = function () {
        ...
        var widgetZone_el = friendlyIframe.parentElement.getElementsByClassName("widget_zone")[0];
        ...
        function setQuizKeys(titles) {
            ...
            for (var i = 0; otherContentOptionContainer_el.length - 1 > i; i++) {
                ...
                var thumbnailImage = firstRandomQuestion ? "url(" + imageUrl + firstRandomQuestion.metadata.image + "4_3.jpg?quality=90&width=" + 70*pixelRatio + ")" : '';
        ...
        function setData() {
            try {
                ...
                //TODO removed from businiess decision
                // if (isActive) {
                ...
                var resultDisplay = activeQuestion ? metaData.correct_answer_result : 'Whoops!',
                    backgroundImage = activeQuestion ? "url(" + imageUrl + metaData.image + "16_9.jpg?width=" + triviaImage_el.offsetWidth*pixelRatio  + ")" : '';
                ...
                if (swapImage && wideWidget) {
                    ...
                    if (!friendlyIfWindow.document.getElementById('trivia_image_style')) {
                        ...
                        imageStyle.innerHTML = '@media (max-width: 649px) {.trivia_image {background-image: url("' + imageUrl + metaData.image + '4_3.jpg?width=' + triviaImage_el.offsetWidth*pixelRatio + '")}}' +
                            '@media (min-width: 650px) {.trivia_image {background-image: url("' + imageUrl + metaData.image + '16_9.jpg?width=' + triviaImage_el.offsetWidth*pixelRatio  + '")}}';
                        imageStyle.id = 'trivia_image_style';
                        friendlyIfWindow.document.getElementsByTagName('section')[0].appendChild(imageStyle);
                    } else {
                        friendlyIfWindow.document.getElementById('trivia_image_style').innerHTML = '@media (max-width: 649px) {.trivia_image {background-image: url("' + imageUrl + metaData.image + '4_3.jpg?width=' + triviaImage_el.offsetWidth*pixelRatio + '")}}' +
                            '@media (min-width: 650px) {.trivia_image {background-image: url("' + imageUrl + metaData.image + '16_9.jpg?width=' + triviaImage_el.offsetWidth*pixelRatio + '")}}';
                    }
        ...
        function setSize() {
          if (!triviaAdZone_el) {
            triviaAdZone_el = document.getElementById('trivia_ad_zone');
          }
            var getWidth;
            if (this === top) {
              // we arent in an iframe
              getWidth = friendlyIframe.parentElement.offsetWidth;
            }
            ...
            if (wideWidget) {
                ...
                if (!isSmall) {
                    adControl(false);
                    widgetZone_el.classList.add("is_big");
                    widgetZone_el.classList.remove("is_small");
                }
                if (isSmall) {
                    triviaAdZone_el.style.display = 'none';
                    widgetZone_el.style.display = 'none';
                    widgetZone_el.classList.remove("is_big");
                    widgetZone_el.classList.add("is_small");
                    ...
                    widgetZone_el.style.display = 'block';
        ...
        //WIDE WIDGET CODE
        if (wideWidget) {
            var adRotationTime = 10; // flip ad vs widget every X seconds
            // wait for DOM to render
            setTimeout(function(){
              // show ad for X seconds initially, if mobile breakpoint
              if (isSmall) {
                intervalScoreContainer_el.style.visibility = 'hidden';
                progressBar_el.style.visibility = 'hidden';
                progressFill_el.style.visibility = 'hidden';
                adControl(false, adRotationTime);
                isAdVisible = false;
              }
              else {
                isAdVisible = false;
            ...
            window.setInterval(function () { // create interval to swap the ad every X secs if the widget has not been engaged
                if (isSmall && total_clicks === 0 && !widgetEngaged && !triviaStarted) {
                    if (!isAdVisible) {
                        intervalScoreContainer_el.style.visibility = 'visible';
                        progressBar_el.style.visibility = 'visible';
                        progressFill_el.style.visibility = 'visible';
                        adControl(true, adRotationTime);
                        isAdVisible = true;
                    } else {
                        intervalScoreContainer_el.style.visibility = 'hidden';
                        progressBar_el.style.visibility = 'hidden';
        ...
        function adControl(isPaused, adTimer) {
            if (!adProgressBar_el) {
              adProgressBar_el = document.getElementById("ad_progress_bar");
            }
            if (!adTimer) {
              adIntervalSeconds = 3; // default to 3 seconds, if not specified
            }
            else {
              adIntervalSeconds = adTimer;
            }
            if (isPaused) {
        ...
        function updatePayload(send) {
            //TODO currently return; from business decision
            return;
