(function () {
    ...
    slider = function (element) {
        ...
        function populateslider(data, loadMore) {
            ...
            //create function to append specific styles for
            createPublisherStyle(currentPub.hex);
            ...
            for (var i = 0; i < items.length && i < items.length; i++) {
                ...
                //image check
                if (data.category == 'weather' || image == null || image == "" || image.indexOf("no_") != -1 || image.indexOf("no-") != -1 || image.indexOf("fallback") != -1) {
        ...
        function appendNewList(appendCount) {
            ...
            lazyLoaded = false;
            loadData(apiCallUrl, true, appendCount);
        }

        //#10-1
        function createPublisherStyle(hex) {
            // log('#10-1 createPublisherStyle', payloadStyles);
            if (!sliderDocument.getElementById('slider_publisher')) {
                var pubStyle = sliderDocument.createElement("style");
                pubStyle.type = "text/css";
                pubStyle.id = "slider_publisher";
                var textStyle = ".profile_image_div.fallback::before  {background-color: " + hex + "}" +
                    ".title_image_div.overlay::before     {background-color: " + hex + "}";

                pubStyle.innerHTML = textStyle;
                sliderDocument.body.appendChild(pubStyle);
            }
        }

        //#10-2
        function createTitleTile(titleData) {
            // log('#10-2 createTitleTile', payloadStyles);
            var sliderBlock = sliderDocument.createElement('div');
            sliderBlock.className = 'slider_block';
            var titleOutput;
            try {
                tileOutput =
                    "<div class='title_overlay' style='background-color: " + titleData.hex + "'>" +
                    "<div class='title_image_div overlay'>" +
        ...
        function createDataTile(type, tileData) {
            // log('#10-3 createTileType', payloadStyles);
            var sliderBlock = sliderDocument.createElement('div');
            sliderBlock.className = 'slider_block';
            var tileOutput;
            try {
