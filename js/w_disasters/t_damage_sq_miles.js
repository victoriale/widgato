...
$(function(){
  ...
  function dataCall(index){
      ...
      if(curData[0].SqMilesDamage == 1){
        $('.fcw-content1').html(Number(curData[0].SqMilesDamage).toFixed(2) + ' Mile');
      } else {
        $('.fcw-content1').html(Number(curData[0].SqMilesDamage).toFixed(2) + ' Miles');
      }
      $('.fcw-content2').html(curData[0].Month + ' ' + curData[0].Day + ', ' + curData[0].Year);
      $('.fcw-presentedby').html('BIGGEST DAMAGE SQ MILES TORNADO- PRESENTED BY');
