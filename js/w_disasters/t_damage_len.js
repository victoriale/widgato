...
$(function(){
  ...
  function dataCall(index){
      ...
      $('.fcw-t1').html('Toradoes with the Biggest Damage Length');
      $('.fcw-t2-loc').html(curData[0].City+', '+curData[0].State);
      $('.fcw-image').css('background', 'url('+imageUrl(curData[0].img)+') no-repeat');
      $('.fcw-img2').html('#'+(index+1));
      if(curData[0].MilesLengthDamaged == 1){
        $('.fcw-content1').html(curData[0].MilesLengthDamaged + ' Mile of Damage');
      } else {
        $('.fcw-content1').html(curData[0].MilesLengthDamaged + ' Miles of Damage');
      }
      $('.fcw-content2').html(curData[0].Month + ' ' + curData[0].Day + ', ' + curData[0].Year);
      $('.fcw-presentedby').html('TORNADO WITH BIGGEST MILES OF DAMAGE - PRESENTED BY');
