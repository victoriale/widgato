...
$(function(){
  ...
  function dataCall(index){
      ...
      $('#title').html('Highest Risk of Having Tornado');
      $('.fcw-t1').html('Cities with the Highest Risk of Experiencing a Tornado');
      $('.fcw-t2-loc').html(curData[0].City+', '+curData[0].State);
      $('.fcw-image').css('background', 'url('+imageUrl(curData[0].img)+') no-repeat');
      $('.fcw-img2').html('#'+(index+1));
      if(curData[0].NumOfTornadoes == 1){
        $('.fcw-content1').html(curData[0].NumOfTornadoes + ' Tornado');
      } else {
        $('.fcw-content1').html(curData[0].NumOfTornadoes + ' Tornadoes');
      }
      $('.fcw-content2').html('Per Year');
      $('.fcw-presentedby').html('CITIES WITH HIGHEST TORNADO RISK - PRESENTED BY');
