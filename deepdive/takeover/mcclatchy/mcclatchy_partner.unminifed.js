(function(){
  ...
  switch(domain){
    case 'bellinghamherald.com':
      railMarginTop = topWin.document.querySelector('header[role="banner"]').offsetHeight;
      contentMaxWidth = 1180;
      contentMarginTop = 0;
      partnerState = 'wa';
      contentEl = topWin.document.querySelector('section.container[role="main"]');
    break;
    case 'bnd.com':
      railMarginTop = topWin.document.querySelector('header[role="banner"]').offsetHeight;
      contentMaxWidth = 1180;
  ...
  topWin.addEventListener('resize', function(){
    ...
    //If rails dont exist and body is big enough for rails, build rails (margin around content is greater than 0 to allow space for skyscraper ads)
    if(!railsLoaded && (resizeBodyWidth - resizeContentWidth) >= 320){
      buildRails();
    }
    //If deep dive does not exist and body is big enough, build deep dive
    if(!deepDiveLoaded && resizeContentWidth >= 990){
      buildDeepDive();
    }
