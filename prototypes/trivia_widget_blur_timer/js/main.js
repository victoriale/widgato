    ...
    function resetPixelationFn() {
        ...
        // set staring point for pixelation
        ctx.drawImage(pixelatedImage, 0, 0, blurWidth, blurHeight);
        ctx.drawImage(pixelateContainer, 0, 0, blurWidth, blurHeight, 0, 0, pixelatedContainerWidth, pixelatedContainerHeight);
    } //resetPixelationFn
    ...
    function adjustPixelationFn(v) {
        ...
        pixelationInterval = setInterval(function() {
            ...
            intervalWidth = intervalWidth + (pixelatedContainerWidth/pixelationDivision) >= pixelatedContainerWidth ? pixelatedContainerWidth : intervalWidth + (pixelatedContainerWidth/pixelationDivision);
            intervalHeight = intervalWidth + (pixelatedContainerWidth/pixelationDivision) >= pixelatedContainerHeight ? pixelatedContainerHeight : intervalHeight + (pixelatedContainerHeight/pixelationDivision);
    ...
    //removes pixelation
    function removePixelationFn() {
        blurWidth = pixelatedContainerWidth;
        blurHeight = pixelatedContainerHeight;
        drawImageProp(ctx, pixelatedImage, 0, 0, blurWidth, blurHeight);
    } //removePixelationFn



    //function is used to imitate backgroundsize cover on canvas element
    function drawImageProp(ctx, img, x, y, w, h, offsetX, offsetY) {
        ...
        if (cx < 0) cx = 0;
        if (cy < 0) cy = 0;
        // if (cw >= iw) { cw = Math.round(iw); ch = Math.round(cw/ratio1) };
        // if (ch >= ih)  { ch = Math.round(ih); cw = Math.round(ch*ratio1) };
        cw = cw;
        ch = ch;
