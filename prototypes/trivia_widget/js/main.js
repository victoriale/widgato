...
triviaWidget = function() {
    var htmlFile = '@@import ../index.html';
    var cssFile = '@@import ../styles/styles.css';
    var cssMobileFile = '@@import ../styles/mobile_styles.css';

    var embedURL = "trivia_widget";
    ...
    createFriendlyIframe(); //create friendly iframe
...
//create an iframe for post request that will be removed once the request has been sent
    function createPostRequestIframe(jsonObject) {
        ...
        //create friendly iframe to place ourselves inside
        var friendlyIframeRequest = document.createElement('iframe');
        // friendlyIframe.id = "friendlyIframe_" + countSelf.length;
        var uniqueIframeId = "snt_product_id_" + rand_id;
        friendlyIframeRequest.setAttribute("id", uniqueIframeId);
        friendlyIframeRequest.className = "report";
        friendlyIframeRequest.width = 1;
        friendlyIframeRequest.height = 1; //250 is the add height
        friendlyIframeRequest.scrolling = 'no';
        friendlyIframeRequest.style.overflow = 'hidden';
        friendlyIframeRequest.src = 'about:blank';
        friendlyIframeRequest.style.border = 'none';
        document.body.appendChild(friendlyIframeRequest);
        friendlyIframeWindow = friendlyIframeRequest.contentWindow;

        //create inline html for friendlyIframeRequest
        friendlyIframeWindow.document.open();
